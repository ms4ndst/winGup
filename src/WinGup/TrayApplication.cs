...
public class TrayApplication : IDisposable
{
    ...
    private static Icon GenerateIcon(int count)
    {
        ...
        var brush = count > 0 ? Brushes.Red : Brushes.Green;
        graphics.FillEllipse(brush, 0, 0, size - 1, size - 1);
        graphics.DrawEllipse(Pens.Black, 0, 0, size - 1, size - 1);
    ...
    private void CheckNow()
    {
        _ = Task.Run(async () =>
        {
            try
            {
                ...
                var response = await _ipcClient.SendMessageAsync("check_updates").ConfigureAwait(false);
                _logger.LogDebug("Check response: {Response}", response);
            }
            catch (Exception ex)
            {
    ...
    private void OnCheckCompleted(object? sender, EventArgs e)
        => InvokeOnUiThread(() => UpdateIcon(_updateChecker!.UpdateCount));
